...
describe Rack::Logstash do
  ...
  before :each do
    expect(Rack::Logstash::Transport).to receive(:new).with('tcp://logstash:5151').and_return(mock_transport)

    expect(mock_transport).to receive(:send) do |argument|
      log_entry.replace(JSON.parse(argument.to_json))
    end
  ...
  context 'a simple request' do
    before :each do
      expect(Time).to receive(:now).with(no_args).and_return(
        Time.at(1_234_567_890).utc,
        Time.at(1_234_567_890.01).utc
      ).at_least(2)
    ...
    it 'logs the server\'s PID' do
      expect(log_entry['pid']).to eq($PROCESS_ID)
    end

    it 'logs the server\'s program' do
      expect(log_entry['program']).to eq($PROGRAM_NAME)
    end
  ...
  context 'a simple request over IPv6' do
    before :each do
      get '/', {}, 'SERVER_PROTOCOL' => 'HTTP/1.1', 'REMOTE_ADDR' => '2001:db8::feed:b00f'
  ...
  context 'with tags' do
    let(:app) do
      ...
      Rack::Builder.new do
        use Rack::Logstash, url, tags: %w[foo bar]
    ...
    it 'sends the tags' do
      expect(log_entry['tags']).to eq(%w[foo bar])
    end
  end
  ...
  context 'with custom fields' do
    let(:app) do
      ...
      Rack::Builder.new do
        use Rack::Logstash, url, custom: { source: 'sample' }
    ...
    it 'sends the customer field' do
      expect(log_entry['source']).to eq('sample')
    end
  end
